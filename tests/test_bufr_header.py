...
def test_header_accessors():
    ...
    with pt.raises(KeyValueNotFoundError): bufr.header['year']
    with pt.raises(KeyValueNotFoundError): bufr.header['year->code']
    with pt.raises(KeyValueNotFoundError): bufr.header['year->percentConfidence']
    with pt.raises(KeyValueNotFoundError): bufr.header['year->percentConfidence->code']
    with pt.raises(KeyValueNotFoundError): bufr.header['year'] = 2025
    with pt.raises(KeyValueNotFoundError): bufr.header['year->percentConfidence'] = 100

def test_header_get_count():
    bufr = BUFRMessage("BUFR3_local")
    assert bufr.get_count('edition') == 1
    with pt.raises(KeyValueNotFoundError): bufr.header.get_count('year')
    with pt.raises(KeyValueNotFoundError): bufr.header.get_count('#1#year')

def test_header_is_missing():
    bufr = BUFRMessage('BUFR3_local')
    assert not bufr.is_missing('edition')
    with pt.raises(KeyValueNotFoundError): bufr.header.is_missing('year')

def test_header_set_missing():
    bufr = BUFRMessage('BUFR3_local')
    with pt.raises(ValueCannotBeMissingError): bufr.set_missing('edition')
    with pt.raises(KeyValueNotFoundError): bufr.header.set_missing('year')
...
def test_header_computed_keys():
    ...
    assert bufr['typicalDateTime'] == np.datetime64('2000-01-02T03:04')
    with pt.raises(NotFoundError): bufr['localDateTime']
    bufr = BUFRMessage('BUFR3_local')
    bufr['localYear'] = 2000
    bufr['localMonth'] = 1
    bufr['localDay'] = 2
