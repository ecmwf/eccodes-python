...
def test_data_key():
    ...
    assert Key.from_string('year->percentConfidence') == Key(name='year->percentConfidence', primary='year', secondary='percentConfidence')
    assert Key.from_string('year->percentConfidence->code') == Key(name='year->percentConfidence', primary='year', secondary='percentConfidence', attribute='code')
    assert Key.from_string('#1#year') == Key(name='year', rank=1)
    assert Key.from_string('#1#year->code') == Key(name='year', rank=1, attribute='code')
    assert Key.from_string('#1#year->percentConfidence') == Key(name='year->percentConfidence', rank=1, primary='year', secondary='percentConfidence')
    assert Key.from_string('#1#year->percentConfidence->code') == Key(name='year->percentConfidence', rank=1, primary='year', secondary='percentConfidence', attribute='code')
    ...
    assert 'year->percentConfidence' == Key(name='year->percentConfidence', primary='year', secondary='percentConfidence').string
    assert 'year->percentConfidence->code' == Key(name='year->percentConfidence', primary='year', secondary='percentConfidence', attribute='code').string
    assert '#1#year' == Key(name='year', rank=1).string
    assert '#1#year->code' == Key(name='year', rank=1, attribute='code').string
    assert '#1#year->percentConfidence' == Key(name='year->percentConfidence', rank=1, primary='year', secondary='percentConfidence').string
    assert '#1#year->percentConfidence->code' == Key(name='year->percentConfidence', rank=1, primary='year', secondary='percentConfidence', attribute='code').string
    with pt.raises(NotFoundError): Key.from_string('/pressure=85000/airTemperature')
...
def test_data_accessors():
    ...
    with pt.raises(NotFoundError): bufr.data['edition']
    with pt.raises(NotFoundError): bufr.data['edition'] = 3
    # Data keys: defined
    ...
    assert np.all(bufr.data['cloudAmount'] == np.ma.masked_values([8, -1, -1, -1, -1], -1))
  ...
  # assert bufr.data['#1#year->percentConfidence->code'] == TODO
    ...
    with pt.raises(ReadOnlyError): bufr.data['year->code'] = 0
    with pt.raises(ReadOnlyError): bufr.data['#1#year->code'] = 0
  # with pt.raises(ReadOnlyError): bufr.data['#1#year->percentConfidence->code'] = TODO
    ...
    with pt.raises(IndexError): bufr.data[-4]

def test_data_get_count():
    bufr = BUFRMessage(open('./sample-data/synop.bufr'))
    with pt.raises(KeyValueNotFoundError): bufr.data.get_count('edition')
    ...
    with pt.raises(KeyValueNotFoundError): bufr.get_count('#2#year')
    ...
    with pt.raises(KeyValueNotFoundError): bufr.get_count('#6#cloudAmount')
    ...
    with pt.raises(KeyValueNotFoundError): bufr.data.get_count('#11#opticalDepth')
    with pt.raises(KeyValueNotFoundError): bufr.data[0].get_count('#3#opticalDepth')
    with pt.raises(KeyValueNotFoundError): bufr.data[1].get_count('#9#opticalDepth')

def test_data_is_missing():
    bufr = BUFRMessage('BUFR3_local_satellite')
    bufr['numberOfSubsets'] = 3
    with pt.raises(KeyValueNotFoundError): bufr.data.is_missing('edition')
...
def test_data_set_missing():
    bufr = BUFRMessage('BUFR3_local_satellite')
    bufr['numberOfSubsets'] = 3
    with pt.raises(KeyValueNotFoundError): bufr.data.set_missing('edition')
    with pt.raises(ValueCannotBeMissingError): bufr.set_missing('pressure->units')
