...
def test_get_datetime_with_decimal_seconds():
    ...
    assert datetime[1] is np.ma.masked
    assert np.ma.all(datetime == np.array(['2000-01-02T03:04:05.6', 'NaT'], dtype='M8[ms]'))
...
def test_get_datetime_with_microseconds():
    data = dict(year=2000, month=1, day=2, hour=3, minute=4, secondsWithinAMinuteMicrosecond=5.6789)
    datetime = get_datetime(data)
    assert not isinstance(datetime, np.ndarray)
    assert datetime == numpy.datetime64('2000-01-02T03:04:05.6789')

def test_get_datetime_with_seconds_and_microseconds():
    data = dict(year=2000, month=1, day=2, hour=3, minute=4, second=5, secondsWithinAMinuteMicrosecond=5.6789)
    datetime = get_datetime(data)
    assert not isinstance(datetime, np.ndarray)
    assert datetime == numpy.datetime64('2000-01-02T03:04:05.6789')
...
def test_get_datetime_with_rank():
    ...
    assert isinstance(datetime, np.ndarray)
    assert np.all(datetime == np.array(['2001-02-03T04:05:06', '2002-03-04T05:06:07'], dtype='M8[s]'))

# Test missing_of()

def test_missing_of():
    with pt.raises(ValueError):  missing_of(np.int8)
    with pt.raises(ValueError):  missing_of(np.int16)
    assert CODES_MISSING_LONG == missing_of(np.int32)
    assert CODES_MISSING_LONG == missing_of(np.int64)
    with pt.raises(ValueError):  missing_of(np.dtype('i2'))
    with pt.raises(ValueError):  missing_of(np.dtype('f4'))
    ...
    assert CODES_MISSING_LONG == missing_of(1)
    with pt.raises(ValueError):    missing_of(np.float16)
    with pt.raises(ValueError):    missing_of(np.float32)
