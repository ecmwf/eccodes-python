...
def grib_get_string(msgid, key):
    ...
    err = lib.grib_get_string(handle, key.encode(ENC), values, length_p)
    GRIB_CHECK(err)
...
def grib_get_string_length(msgid, key):
    ...
    handle = ffi.cast('grib_handle *', msgid)
    size = ffi.new('size_t *')
    err = lib.grib_get_length(handle, key.encode(ENC), size)
    GRIB_CHECK(err)
    return size[0]
...
def grib_get_long(msgid, key):
    ...
    err = lib.grib_get_long(handle, key.encode(ENC), value_p)
    GRIB_CHECK(err)
    return value_p[0]
...
def grib_get_double(msgid, key):
    ...
    err = lib.grib_get_double(handle, key.encode(ENC), value_p)
    GRIB_CHECK(err)
    return value_p[0]
...
def grib_get_native_type(msgid, key):
    ...
    handle = ffi.cast('grib_handle *', msgid)
    itype_p = ffi.new('int*')
    err = lib.grib_get_native_type(handle, key.encode(ENC), itype_p)
    GRIB_CHECK(err)
