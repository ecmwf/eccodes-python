...
class Coder:
    ...
    def __init__(self, source) -> None:
        if isinstance(source, io.IOBase):
            try:
                ...
                fileno = None
            if fileno is None:
                raise TypeError('Expected file-like object with a file descriptor; got %s' % type(source))
            ...
            raise TypeError('Expected file-like object, bytes array or a sample name; got %s' % type(source))
        self._handle = handle
    ...
    def _extract(self, subsets):
        ...
        if is_interval:
            ...
            method = 'SubsetList'
            codes_set_long_array(self._clone_handle, 'extractSubsetList', [s+1 for s in subsets])
    ...
    def checkout(self, entry: DataEntry):
        if entry.flags & FACTOR:
            ...
            array = codes_get_array(self._handle, entry.name)
            entry.shape = (entry.shape[0], array.size) # infer the 2nd dimension earlier TODO
                ...
                for rank, is_set in enumerate(rank_mask, start=1):
                    if is_set:
                        ...
                        if not self._compressed or entry.flags & SCALAR:
                            array[rank-1] = codes_get(self._handle, f'#{rank}#{entry.name}')
                        else:
                            array[rank-1,:] = codes_get_array(self._handle, f'#{rank}#{entry.name}')
                    else:
                        array[rank-1] = fill_value
        elif entry.uniform_element \
                and entry.uniform_element.code == 31021: # [0]
            array = np.empty(entry.shape, int)
            if not self._compressed or entry.flags & SCALAR:
                for rank in range(1, entry.shape[0] + 1):
                    array[rank-1] = codes_get_long(self._handle, f'#{rank}#{entry.name}')
            else:
                for rank in range(1, entry.shape[0] + 1):
                    array[rank-1,:] = codes_get_long_array(self._handle, f'#{rank}#{entry.name}')
        else:
            ktype = None
            ...
            if isinstance(array_or_list, list) and isinstance(array_or_list[0], str):
                if not entry.uniform_element:
                    raise NotImplementedError('Non-uniform string keys are not supported yet: %s' % entry.name)
            ...
            if array.dtype.type == np.str_:
                ...
                for rank in range(1, rank_count + 1):
                    ...
                    if np.any(rank_array != rank_array[0]):
                        ...
                        codes_set_array(self._handle, f'#{rank}#{key}', rank_array[0:1]) # [2]
            else:
                if key == 'centre': # [3]
    ...
    def get(self, key: str, header_only=False, data_only=False, validate=True) -> ValueLike:
        if validate:
            ...
            if header_only:
                ...
                if key == 'centre':
                    raise NotFoundError(f"{key}: Did you mean bufrHeaderCentre? (see ECC-1624)")
                if '->' in key:     # [1]
                    raise NotFoundError(key)
            elif data_only:
    ...
    def get_tables(self) -> Tables:
        version = Version()
        version.master = cast(int, codes_get_long(self._handle, 'masterTablesVersionNumber'))
        version.local  = cast(int, codes_get_long(self._handle, 'localTablesVersionNumber'))
        version.centre = cast(int, codes_get_long(self._handle, 'bufrHeaderCentre'))
        version.subcentre = cast(int, codes_get_long(self._handle, 'bufrHeaderSubCentre'))
    ...
    def is_missing(self, entry: DataEntry, slice: slice) -> bool:
        ...
        for rank in range(slice.start + 1, slice.stop + 1):
            if not (is_missing := codes_is_missing(self._handle, f'#{rank}#{entry.name}')):
                break
    ...
    def keys(self, header_only=False, data_only=False, **kwargs) -> Iterator[str]:
        assert not (header_only and data_only)
        yield from keys_of(self._handle, header_only=header_only, data_only=data_only, **kwargs)
    ...
    def set(self, key: str, value: ValueLike, header_only=False, data_only=False,
        ...
        if validate:
            ...
            if key in ('pack', 'unpack'):
                message = "%sing via the '%s' key is disallowed. Call %s() method istead."
...
def keys_of(msg_handle, bufr_only=True, header_only=False, data_only=False,
    ...
    """
    if header_only and data_only:
        raise ValueError("Keyword arguments `header_only` and `data_only` can't both be True")
