...
def get_time(view: Union['View', abc.Mapping],
    ...
    for name, dtype in zip(names, dtypes):
        ...
        parts.append(part)
    if not prefix:
        key = Key('secondsWithinAMinuteMicrosecond', rank if isinstance(rank, int) else None)
        try:
            value = view[key.string]
...
def set_datetime(view: Union['View', abc.MutableMapping], value: Union[DateLike, np.ndarray],
        rank: Optional[int] = None, prefix: str = '') -> None:
    set_date(view, value, rank, prefix)
    set_time(view, value, rank, prefix)

def set_date(view: Union['View', abc.MutableMapping], value: Union[DateLike, np.ndarray],
        rank: Optional[int] = None, prefix: str = '') -> None:
    names = ['year', 'month', 'day']
    keys = []
...
def ensure_array(value: Any, dtype: Optional[DTypeLike] = None) -> Union[NDArray, MaskedArray]:
